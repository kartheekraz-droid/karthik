...
type Role = 'central' | 'state' | 'collector' | 'pia' | 'field' | 'citizen'

const ROLES: { code: Role; label: string; to: string }[] = [
  { code: 'central', label: 'Central Govt.', to: '/central' },
  { code: 'state', label: 'State Govt.', to: '/state' },
  { code: 'collector', label: 'Collector', to: '/collector' },
  { code: 'pia', label: 'PIA', to: '/pia' },
  { code: 'field', label: 'Field Officer', to: '/field' },
  { code: 'citizen', label: 'Citizen', to: '/citizen' },
]
...
export default function Login() {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    // then read the role from the `profiles` table instead of the local dropdown.
    const dest = ROLES.find((r) => r.code === role)?.to ?? '/'
    navigate(dest)
  }
      ...
      <div className="grid grid-cols-2 min-h-[calc(100vh-58px)] max-[900px]:grid-cols-1">
        ...
        <section className="grid place-items-center p-10">
          <form
            ...
            className="w-[min(460px,100%)] bg-white border border-line rounded-[18px] p-7 shadow-card"
          >
            ...
            <div className="grid grid-cols-3 gap-2">
              {ROLES.map((r) => (
                <label
                  key={r.code}
                  className={
                    'text-center border-2 rounded-[10px] px-1.5 py-2.5 cursor-pointer text-xs font-semibold ' +
                    (role === r.code ? 'border-navy text-navy' : 'border-line')
                  }
                >
                  <input
                    type="radio"
